...
''' <summary>
'''Контрол редактирования раздела Комментарии к проверке режим:adm
''' </summary>
''' <remarks>
'''
''' </remarks>
Public Class editto_taskcheckcommentadm
...
Dim OK as boolean 
        If GuiManager.GetReferenceDialog("to_oper","",System.guid.Empty, id, brief) Then
          txtthe_operator.Tag = id
          txtthe_operator.text = brief
        End If
...
If not txtthe_operator.Tag.Equals(System.Guid.Empty) Then
  item.the_operator = Item.Application.FindRowObject("to_oper",txtthe_operator.Tag)
Else
   item.the_operator = Nothing
End If
  try
    item.the_date = dtpthe_date.value
  catch
    item.the_date = System.DateTime.MinValue
  end try
