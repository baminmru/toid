...
'''
''' </remarks>
Public Class editto_taskchecksadm
...
Dim brief As String = string.Empty
Dim OK as boolean 
        If GuiManager.GetReferenceDialog("to_cardchecks","",System.guid.Empty, id, brief) Then
          txtcheckref.Tag = id
          txtcheckref.text = brief
        End If
        catch ex as System.Exception
        Debug.Print(ex.Message +" >> " + ex.StackTrace)
        end try
...
If not txtcheckref.Tag.Equals(System.Guid.Empty) Then
  item.checkref = Item.Application.FindRowObject("to_cardchecks",txtcheckref.Tag)
Else
   item.checkref = Nothing
End If
item.tagid = txttagid.text
  if dtptagtime.checked=false then
       item.tagtime = System.DateTime.MinValue
  else 
  try
    item.tagtime = dtptagtime.value
  catch
    item.tagtime = System.DateTime.MinValue
  end try
  end if
