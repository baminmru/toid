...
''' <summary>
'''Контрол редактирования раздела Описание режим:adm
''' </summary>
''' <remarks>
'''
''' </remarks>
Public Class editto_taskinfoadm
...
Dim OK as boolean 
        If GuiManager.GetReferenceDialog("to_scheditems","",System.guid.Empty, id, brief) Then
...
If not txtthemachine.Tag.Equals(System.Guid.Empty) Then
  item.themachine = Item.Application.FindRowObject("to_scheditems",txtthemachine.Tag)
Else
...
End If
  try
    item.crdate = dtpcrdate.value
  catch
    item.crdate = System.DateTime.MinValue
  end try
   item.taskfinished = cmbtaskfinished.SelectedValue
  if dtpfinishtime.checked=false then
       item.finishtime = System.DateTime.MinValue
  else 
  try
    item.finishtime = dtpfinishtime.value
  catch
    item.finishtime = System.DateTime.MinValue
  end try
  end if
