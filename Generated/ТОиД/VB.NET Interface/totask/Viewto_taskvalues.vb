...
Public Class viewto_taskvalues
...
''' </remarks>
    Public Sub Init()
        ...
        Dim dt As DataTable
        dt = item.to_taskvalues.GetDataTable()
        dt.TableName = "to_taskvalues"

        ts = New DataGridTableStyle
        ts.MappingName = "to_taskvalues"
        ts.ReadOnly = True
        ...
        cs.ReadOnly = True
        cs.HeaderText = "Значение для проверки"
        cs.MappingName = "checkvalue"
        cs.NullText = ""
...
''' </remarks>
    Private Sub gv_OnEdit(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridEdit
        Dim ed As totask.totask.to_taskvalues
        ed = item.FindRowObject("to_taskvalues", ID)
    ...
    Private Sub gv_OnAdd(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridAdd
      If not mReadOnly Then
        ...
        If ID.Equals(System.guid.Empty) Then
              ed = Item.to_taskvalues.Add
          Else
              ed = Item.to_taskvalues.Add(ID.ToString())
          End If
        Dim gui As Doc_GUIBase
        Dim dt As DataTable
        gui = GuiManager.GetTypeGUI(ed.Application.TypeName)
