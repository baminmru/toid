...
''' </remarks>
    Public Sub Init()
        ...
        Dim dt As DataTable
        dt = item.toimg_data.GetDataTable()
        dt.TableName = "toimg_data"

        ts = New DataGridTableStyle
        ts.MappingName = "toimg_data"
        ts.ReadOnly = True
        ...
        cs = New DataGridTextBoxColumn
        cs.ReadOnly = True
        cs.HeaderText = "Раздел привязки"
        cs.MappingName = "link2part"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)

        cs = New DataGridTextBoxColumn
        cs.ReadOnly = True
        cs.HeaderText = "Идентификатор привязки"
        cs.MappingName = "link2id"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)

        cs = New DataGridTextBoxColumn
        cs.ReadOnly = True
        cs.HeaderText = "Тип файла"
        ...
        cs = New DataGridTextBoxColumn
        cs.ReadOnly = True
        cs.HeaderText = "Раздел2"
        cs.MappingName = "link1part"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)

        cs = New DataGridTextBoxColumn
        cs.ReadOnly = True
        cs.HeaderText = "Идентификатор2"
        cs.MappingName = "link1id"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)
...
''' </remarks>
    Private Sub gv_OnEdit(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridEdit
        Dim ed As toimg.toimg.toimg_data
        ed = item.FindRowObject("toimg_data", ID)
    ...
    Private Sub gv_OnAdd(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridAdd
      If not mReadOnly Then
        ...
        If ID.Equals(System.guid.Empty) Then
              ed = Item.toimg_data.Add
          Else
              ed = Item.toimg_data.Add(ID.ToString())
          End If
        Dim gui As Doc_GUIBase
        Dim dt As DataTable
        gui = GuiManager.GetTypeGUI(ed.Application.TypeName)
