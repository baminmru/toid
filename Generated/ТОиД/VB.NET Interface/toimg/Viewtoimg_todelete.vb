...
Public Class viewtoimg_todelete
...
''' </remarks>
    Public Sub Init()
        ...
        Dim dt As DataTable
        dt = item.toimg_todelete.GetDataTable()
        dt.TableName = "toimg_todelete"

        ts = New DataGridTableStyle
        ts.MappingName = "toimg_todelete"
        ts.ReadOnly = True
        ...
        cs = New DataGridTextBoxColumn
        cs.ReadOnly = True
        cs.HeaderText = "ид строки дя удаления"
        cs.MappingName = "filerowid"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)
...
''' </remarks>
    Private Sub gv_OnEdit(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridEdit
        Dim ed As toimg.toimg.toimg_todelete
        ed = item.FindRowObject("toimg_todelete", ID)
    ...
    Private Sub gv_OnAdd(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridAdd
      If not mReadOnly Then
        ...
        If ID.Equals(System.guid.Empty) Then
              ed = Item.toimg_todelete.Add
          Else
              ed = Item.toimg_todelete.Add(ID.ToString())
          End If
        Dim gui As Doc_GUIBase
        Dim dt As DataTable
        gui = GuiManager.GetTypeGUI(ed.Application.TypeName)
