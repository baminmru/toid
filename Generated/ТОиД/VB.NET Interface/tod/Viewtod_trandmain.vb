...
Public Class viewtod_trandmain
...
''' </remarks>
    Public Sub Init()
        ...
        Dim dt As DataTable
        dt = item.tod_trand.GetDataTable()
        dt.TableName = "tod_trand"

        ts = New DataGridTableStyle
        ts.MappingName = "tod_trand"
        ts.ReadOnly = True
...
''' </remarks>
    Private Sub gv_OnEdit(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridEdit
        Dim ed As tod.tod.tod_trand
        ed = item.FindRowObject("tod_trand", ID)
...
''' </remarks>
    ...
    Exit Sub
        ...
        If ID.Equals(System.guid.Empty) Then
              ed = Item.tod_trand.Add
          Else
              ed = Item.tod_trand.Add(ID.ToString())
          End If
        Dim gui As Doc_GUIBase
        Dim dt As DataTable
        gui = GuiManager.GetTypeGUI(ed.Application.TypeName)
