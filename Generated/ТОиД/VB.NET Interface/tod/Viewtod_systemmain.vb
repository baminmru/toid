...
Public Class viewtod_systemmain
...
''' </remarks>
    Public Sub Init()
        ...
        Dim dt As DataTable
        dt = item.tod_system.GetDataTable()
        dt.TableName = "tod_system"

        ts = New DataGridTableStyle
        ts.MappingName = "tod_system"
        ts.ReadOnly = True
...
''' </remarks>
    Private Sub gv_OnEdit(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridEdit
        Dim ed As tod.tod.tod_system
        ed = item.FindRowObject("tod_system", ID)
...
''' </remarks>
    ...
    Exit Sub
        ...
        If ID.Equals(System.guid.Empty) Then
              ed = Item.tod_system.Add
          Else
              ed = Item.tod_system.Add(ID.ToString())
          End If
        Dim gui As Doc_GUIBase
        Dim dt As DataTable
        gui = GuiManager.GetTypeGUI(ed.Application.TypeName)
