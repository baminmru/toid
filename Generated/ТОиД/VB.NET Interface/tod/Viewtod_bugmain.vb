...
''' </remarks>
    Public Sub Init()
        ...
        Dim dt As DataTable
        dt = item.tod_bug.GetDataTable()
        dt.TableName = "tod_bug"

        ts = New DataGridTableStyle
        ts.MappingName = "tod_bug"
        ts.ReadOnly = True
...
''' </remarks>
    ...
    Exit Sub
        ...
        If ID.Equals(System.guid.Empty) Then
              ed = Item.tod_bug.Add
          Else
              ed = Item.tod_bug.Add(ID.ToString())
          End If
        Dim gui As Doc_GUIBase
        Dim dt As DataTable
        gui = GuiManager.GetTypeGUI(ed.Application.TypeName)
