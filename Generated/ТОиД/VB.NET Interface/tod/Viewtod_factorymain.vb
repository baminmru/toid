...
Public Class viewtod_factorymain
...
''' </remarks>
    Public Sub Init()
        ...
        Dim dt As DataTable
        dt = item.tod_factory.GetDataTable()
        dt.TableName = "tod_factory"

        ts = New DataGridTableStyle
        ts.MappingName = "tod_factory"
        ts.ReadOnly = True
...
''' </remarks>
    Private Sub gv_OnEdit(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridEdit
        Dim ed As tod.tod.tod_factory
        ed = item.FindRowObject("tod_factory", ID)
...
''' </remarks>
    ...
    Exit Sub
        ...
        If ID.Equals(System.guid.Empty) Then
              ed = Item.tod_factory.Add
          Else
              ed = Item.tod_factory.Add(ID.ToString())
          End If
        Dim gui As Doc_GUIBase
        Dim dt As DataTable
        gui = GuiManager.GetTypeGUI(ed.Application.TypeName)
