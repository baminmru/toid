...
Public Class viewtod_oprolemain
...
''' </remarks>
    Public Sub Init()
        ...
        Dim dt As DataTable
        dt = item.tod_oprole.GetDataTable()
        dt.TableName = "tod_oprole"

        ts = New DataGridTableStyle
        ts.MappingName = "tod_oprole"
        ts.ReadOnly = True
        ...
        cs = New DataGridTextBoxColumn
        cs.ReadOnly = True
        cs.HeaderText = "Режим документов"
        cs.MappingName = "docmode"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)
...
''' </remarks>
    Private Sub gv_OnEdit(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridEdit
        Dim ed As tod.tod.tod_oprole
        ed = item.FindRowObject("tod_oprole", ID)
...
''' </remarks>
    ...
    Exit Sub
        ...
        If ID.Equals(System.guid.Empty) Then
              ed = Item.tod_oprole.Add
          Else
              ed = Item.tod_oprole.Add(ID.ToString())
          End If
        Dim gui As Doc_GUIBase
        Dim dt As DataTable
        gui = GuiManager.GetTypeGUI(ed.Application.TypeName)
