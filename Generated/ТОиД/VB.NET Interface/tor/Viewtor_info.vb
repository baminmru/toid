...
#End Region
    Public item As tor.tor.Application
...
''' </remarks>
    Public Sub Attach(ByVal docItem As LATIR2.Document.Doc_Base, ByVal gm As LATIR2GuiManager.LATIRGuiManager, byval ReadOnlyView as boolean )  Implements LATIR2GUIManager.IViewPanel.Attach
        ...
        item = CType(docItem, tor.tor.Application)
        GuiManager = gm
        Init()
    End Sub
...
''' </remarks>
    Public Sub Init()
        ...
        Dim dt As DataTable
        dt = item.tor_info.GetDataTable()
        dt.TableName = "tor_info"

        ts = New DataGridTableStyle
        ts.MappingName = "tor_info"
        ts.ReadOnly = True
        ...
        cs.ReadOnly = True
        cs.HeaderText = "Название станции"
        cs.MappingName = "name"
        cs.NullText = ""
        ...
        cs = New DataGridTextBoxColumn
        cs.ReadOnly = True
        cs.HeaderText = "Дата сервер"
        cs.MappingName = "serverdata"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)

        cs = New DataGridTextBoxColumn
        cs.ReadOnly = True
        cs.HeaderText = "Дата клиент"
        cs.MappingName = "clientdata"
        cs.NullText = ""
        ts.GridColumnStyles.Add (cs)
...
''' </remarks>
    Private Sub gv_OnEdit(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridEdit
        Dim ed As tor.tor.tor_info
        ed = item.FindRowObject("tor_info", ID)
    ...
    Private Sub gv_OnAdd(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridAdd
      If not mReadOnly Then
        ...
        If ID.Equals(System.guid.Empty) Then
              ed = Item.tor_info.Add
          Else
              ed = Item.tor_info.Add(ID.ToString())
          End If
        Dim gui As Doc_GUIBase
        Dim dt As DataTable
        gui = GuiManager.GetTypeGUI(ed.Application.TypeName)
