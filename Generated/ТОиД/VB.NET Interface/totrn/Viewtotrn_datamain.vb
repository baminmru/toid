...
Public Class viewtotrn_datamain
...
''' </remarks>
    Public Sub Init()
        ...
        Dim dt As DataTable
        dt = item.totrn_data.GetDataTable()
        dt.TableName = "totrn_data"

        ts = New DataGridTableStyle
        ts.MappingName = "totrn_data"
        ts.ReadOnly = True
        ...
        cs.ReadOnly = True
        cs.HeaderText = "Временная метка"
        cs.MappingName = "time_label"
        cs.NullText = ""
...
''' </remarks>
    Private Sub gv_OnEdit(ByRef OK As Boolean, ByVal ID As System.Guid) Handles gv.OnGridEdit
        Dim ed As totrn.totrn.totrn_data
        ed = item.FindRowObject("totrn_data", ID)
...
''' </remarks>
    ...
    Exit Sub
        ...
        If ID.Equals(System.guid.Empty) Then
              ed = Item.totrn_data.Add
          Else
              ed = Item.totrn_data.Add(ID.ToString())
          End If
        Dim gui As Doc_GUIBase
        Dim dt As DataTable
        gui = GuiManager.GetTypeGUI(ed.Application.TypeName)
