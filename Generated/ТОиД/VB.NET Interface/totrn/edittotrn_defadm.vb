...
''' <summary>
'''Контрол редактирования раздела Описание тренда режим:adm
''' </summary>
''' <remarks>
'''
''' </remarks>
Public Class edittotrn_defadm
...
Dim brief As String = string.Empty
Dim OK as boolean 
        If GuiManager.GetReferenceDialog("tod_trand","",System.guid.Empty, id, brief) Then
          txttrandtype.Tag = id
          txttrandtype.text = brief
        End If
        catch ex as System.Exception
        Debug.Print(ex.Message +" >> " + ex.StackTrace)
        end try
end sub
        Private Sub txttopvalue_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles txttopvalue.Validating
        If txttopvalue.Text <> "" Then
            try
            If Not IsNumeric(txttopvalue.Text) Then
                e.Cancel = True
                MsgBox("Ожидалось число", vbOKOnly + vbExclamation, "Внимание")
            ElseIf Val(txttopvalue.Text) < -922337203685478# Or Val(txttopvalue.Text) > 922337203685478# Then
                e.Cancel = True
...
private sub txttopvalue_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txttopvalue.TextChanged
  Changing
        ...
        Private Sub txtbottomval_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles txtbottomval.Validating
        If txtbottomval.Text <> "" Then
            try
            If Not IsNumeric(txtbottomval.Text) Then
                e.Cancel = True
                MsgBox("Ожидалось число", vbOKOnly + vbExclamation, "Внимание")
            ElseIf Val(txtbottomval.Text) < -922337203685478# Or Val(txtbottomval.Text) > 922337203685478# Then
                e.Cancel = True
                MsgBox("Значение вне допустимого диапазона", vbOKOnly + vbExclamation, "Внимание")
            End If
        catch ex as System.Exception
...
End If
If not txttrandtype.Tag.Equals(System.Guid.Empty) Then
  item.trandtype = Item.Application.FindRowObject("tod_trand",txttrandtype.Tag)
Else
   item.trandtype = Nothing
End If
item.topvalue = cdbl(txttopvalue.text)
item.bottomval = cdbl(txtbottomval.text)
  end if
  mChanged = false
  raiseevent saved()
end sub
