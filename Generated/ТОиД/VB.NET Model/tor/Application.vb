...
''' <summary>
''' Внутренняя переменная для раздела Записи о рабочих станциях
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_tor_info As tor_info_col


''' <summary>
'''Свойство для доступа к строкам раздела Записи о рабочих станциях
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property tor_info() As tor_info_col
            Get
                If m_tor_info Is Nothing Then
                    m_tor_info = New tor_info_col
                    m_tor_info.Application = Me
                    m_tor_info.Parent = Me
                    m_tor_info.Refresh()
                End If
                tor_info = m_tor_info
            End Get
...
''' </remarks>
        ...
        FindInCollections = Nothing
            ...
            mFindInCollections = tor_info.FindObject(Table, InstID)
...
''' </remarks>
        Protected Overrides Sub XMLLoadCollections(ByVal node As System.Xml.XmlNode, Optional ByVal LoadMode As Integer = 0)
           ...
           try
            e_list = node.SelectNodes("tor_info_COL")
            tor_info.XMLLoad(e_list, LoadMode)
