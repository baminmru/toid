...
''' <summary>
''' Внутренняя переменная для раздела Картинки
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_toimg_data As toimg_data_col


''' <summary>
'''Свойство для доступа к строкам раздела Картинки
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property toimg_data() As toimg_data_col
            Get
                If m_toimg_data Is Nothing Then
                    m_toimg_data = New toimg_data_col
                    m_toimg_data.Application = Me
                    m_toimg_data.Parent = Me
                    m_toimg_data.Refresh()
                End If
                toimg_data = m_toimg_data
            End Get
        End Property


''' <summary>
''' Внутренняя переменная для раздела Файлы на удаление
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_toimg_todelete As toimg_todelete_col


''' <summary>
'''Свойство для доступа к строкам раздела Файлы на удаление
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property toimg_todelete() As toimg_todelete_col
            Get
                If m_toimg_todelete Is Nothing Then
                    m_toimg_todelete = New toimg_todelete_col
                    m_toimg_todelete.Application = Me
                    m_toimg_todelete.Parent = Me
                    m_toimg_todelete.Refresh()
                End If
                toimg_todelete = m_toimg_todelete
            End Get
        End Property


''' <summary>
''' Внутренняя переменная для раздела Описание хранилища
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_toimg_def As toimg_def_col


''' <summary>
'''Свойство для доступа к строкам раздела Описание хранилища
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property toimg_def() As toimg_def_col
            Get
                If m_toimg_def Is Nothing Then
                    m_toimg_def = New toimg_def_col
                    m_toimg_def.Application = Me
                    m_toimg_def.Parent = Me
                    m_toimg_def.Refresh()
                End If
                toimg_def = m_toimg_def
            End Get
        End Property
...
''' </remarks>
        ...
        FindInCollections = Nothing
            ...
            mFindInCollections = toimg_def.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = toimg_data.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = toimg_todelete.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
        End Function
...
''' </remarks>
        Protected Overrides Sub XMLLoadCollections(ByVal node As System.Xml.XmlNode, Optional ByVal LoadMode As Integer = 0)
           ...
           try
            e_list = node.SelectNodes("toimg_def_COL")
            toimg_def.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("toimg_data_COL")
            toimg_data.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("toimg_todelete_COL")
            toimg_todelete.XMLLoad(e_list, LoadMode)
...
''' </remarks>
        Public Overrides Sub XMLSaveCollections(ByVal node As System.Xml.XmlElement, ByVal Xdom As System.Xml.XmlDocument)
            toimg_def.XMLSave(node, Xdom)
            toimg_data.XMLSave(node, Xdom)
            toimg_todelete.XMLSave(node, Xdom)
        End Sub
...
Public Overrides Sub BatchUpdate()
    toimg_def.BatchUpdate
    toimg_data.BatchUpdate
    toimg_todelete.BatchUpdate
End Sub
    End Class
End Namespace
