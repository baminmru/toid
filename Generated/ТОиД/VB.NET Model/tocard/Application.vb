...
'''
''' </remarks>
  Private m_to_cardinfo As to_cardinfo_col
...
'''
''' </remarks>
        Public ReadOnly Property to_cardinfo() As to_cardinfo_col
            Get
                If m_to_cardinfo Is Nothing Then
                    m_to_cardinfo = New to_cardinfo_col
                    m_to_cardinfo.Application = Me
                    m_to_cardinfo.Parent = Me
                    m_to_cardinfo.Refresh()
                End If
                to_cardinfo = m_to_cardinfo
            End Get
        End Property
...
'''
''' </remarks>
  Private m_to_cardchecks As to_cardchecks_col
...
'''
''' </remarks>
        Public ReadOnly Property to_cardchecks() As to_cardchecks_col
            Get
                If m_to_cardchecks Is Nothing Then
                    m_to_cardchecks = New to_cardchecks_col
                    m_to_cardchecks.Application = Me
                    m_to_cardchecks.Parent = Me
                    m_to_cardchecks.Refresh()
                End If
                to_cardchecks = m_to_cardchecks
            End Get
        End Property
...
''' </remarks>
        ...
        FindInCollections = Nothing
            ...
            mFindInCollections = to_cardinfo.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = to_cardchecks.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
        End Function
...
''' </remarks>
        Protected Overrides Sub XMLLoadCollections(ByVal node As System.Xml.XmlNode, Optional ByVal LoadMode As Integer = 0)
           ...
           try
            e_list = node.SelectNodes("to_cardinfo_COL")
            to_cardinfo.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("to_cardchecks_COL")
            to_cardchecks.XMLLoad(e_list, LoadMode)
...
''' </remarks>
        Public Overrides Sub XMLSaveCollections(ByVal node As System.Xml.XmlElement, ByVal Xdom As System.Xml.XmlDocument)
            to_cardinfo.XMLSave(node, Xdom)
            to_cardchecks.XMLSave(node, Xdom)
        End Sub
...
Public Overrides Sub BatchUpdate()
    to_cardinfo.BatchUpdate
    to_cardchecks.BatchUpdate
End Sub
    End Class
End Namespace
