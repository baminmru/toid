...
''' <summary>
''' Внутренняя переменная для раздела Данные тренда
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_totrn_data As totrn_data_col


''' <summary>
'''Свойство для доступа к строкам раздела Данные тренда
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property totrn_data() As totrn_data_col
            Get
                If m_totrn_data Is Nothing Then
                    m_totrn_data = New totrn_data_col
                    m_totrn_data.Application = Me
                    m_totrn_data.Parent = Me
                    m_totrn_data.Refresh()
                End If
                totrn_data = m_totrn_data
            End Get
        End Property


''' <summary>
''' Внутренняя переменная для раздела Описание тренда
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_totrn_def As totrn_def_col


''' <summary>
'''Свойство для доступа к строкам раздела Описание тренда
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property totrn_def() As totrn_def_col
            Get
                If m_totrn_def Is Nothing Then
                    m_totrn_def = New totrn_def_col
                    m_totrn_def.Application = Me
                    m_totrn_def.Parent = Me
                    m_totrn_def.Refresh()
                End If
                totrn_def = m_totrn_def
            End Get
        End Property
...
''' </remarks>
        ...
        FindInCollections = Nothing
            ...
            mFindInCollections = totrn_def.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = totrn_data.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
        End Function
...
''' </remarks>
        Protected Overrides Sub XMLLoadCollections(ByVal node As System.Xml.XmlNode, Optional ByVal LoadMode As Integer = 0)
           ...
           try
            e_list = node.SelectNodes("totrn_def_COL")
            totrn_def.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("totrn_data_COL")
            totrn_data.XMLLoad(e_list, LoadMode)
...
''' </remarks>
        Public Overrides Sub XMLSaveCollections(ByVal node As System.Xml.XmlElement, ByVal Xdom As System.Xml.XmlDocument)
            totrn_def.XMLSave(node, Xdom)
            totrn_data.XMLSave(node, Xdom)
        End Sub
...
Public Overrides Sub BatchUpdate()
    totrn_def.BatchUpdate
    totrn_data.BatchUpdate
End Sub
    End Class
End Namespace
