...
'''
''' </remarks>
  Private m_tod_bug As tod_bug_col


''' <summary>
'''Свойство для доступа к строкам раздела Типичные проблемы
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property tod_bug() As tod_bug_col
            Get
                If m_tod_bug Is Nothing Then
                    m_tod_bug = New tod_bug_col
                    m_tod_bug.Application = Me
                    m_tod_bug.Parent = Me
                    m_tod_bug.Refresh()
                End If
                tod_bug = m_tod_bug
...
''' <summary>
''' Внутренняя переменная для раздела Тип измерения
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_tod_valtype As tod_valtype_col


''' <summary>
'''Свойство для доступа к строкам раздела Тип измерения
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property tod_valtype() As tod_valtype_col
            Get
                If m_tod_valtype Is Nothing Then
                    m_tod_valtype = New tod_valtype_col
                    m_tod_valtype.Application = Me
                    m_tod_valtype.Parent = Me
                    m_tod_valtype.Refresh()
                End If
                tod_valtype = m_tod_valtype
            End Get
        End Property


''' <summary>
''' Внутренняя переменная для раздела Модели станков
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_tod_model As tod_model_col


''' <summary>
'''Свойство для доступа к строкам раздела Модели станков
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property tod_model() As tod_model_col
            Get
                If m_tod_model Is Nothing Then
                    m_tod_model = New tod_model_col
                    m_tod_model.Application = Me
                    m_tod_model.Parent = Me
                    m_tod_model.Refresh()
                End If
                tod_model = m_tod_model
            End Get
        End Property
...
'''
''' </remarks>
  Private m_tod_system As tod_system_col
...
'''
''' </remarks>
        Public ReadOnly Property tod_system() As tod_system_col
            Get
                If m_tod_system Is Nothing Then
                    m_tod_system = New tod_system_col
                    m_tod_system.Application = Me
                    m_tod_system.Parent = Me
...
''' <summary>
''' Внутренняя переменная для раздела Станки
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_tod_st As tod_st_col


''' <summary>
'''Свойство для доступа к строкам раздела Станки
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property tod_st() As tod_st_col
            Get
                If m_tod_st Is Nothing Then
                    m_tod_st = New tod_st_col
                    m_tod_st.Application = Me
                    m_tod_st.Parent = Me
                    m_tod_st.Refresh()
                End If
                tod_st = m_tod_st
...
''' <summary>
''' Внутренняя переменная для раздела Материалы и инструменты
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_tod_material As tod_material_col


''' <summary>
'''Свойство для доступа к строкам раздела Материалы и инструменты
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property tod_material() As tod_material_col
            Get
                If m_tod_material Is Nothing Then
                    m_tod_material = New tod_material_col
                    m_tod_material.Application = Me
                    m_tod_material.Parent = Me
                    m_tod_material.Refresh()
                End If
                tod_material = m_tod_material
            End Get
        End Property


''' <summary>
''' Внутренняя переменная для раздела Единицы измерения
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_tod_edizm As tod_edizm_col


''' <summary>
'''Свойство для доступа к строкам раздела Единицы измерения
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property tod_edizm() As tod_edizm_col
            Get
                If m_tod_edizm Is Nothing Then
                    m_tod_edizm = New tod_edizm_col
                    m_tod_edizm.Application = Me
                    m_tod_edizm.Parent = Me
                    m_tod_edizm.Refresh()
                End If
                tod_edizm = m_tod_edizm
            End Get
        End Property
...
''' </remarks>
        ...
        FindInCollections = Nothing
            ...
            mFindInCollections = tod_model.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = tod_st.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = tod_system.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = tod_edizm.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = tod_bug.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
...
''' </remarks>
        Protected Overrides Sub XMLLoadCollections(ByVal node As System.Xml.XmlNode, Optional ByVal LoadMode As Integer = 0)
           ...
           try
            e_list = node.SelectNodes("tod_model_COL")
            tod_model.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("tod_st_COL")
            tod_st.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("tod_system_COL")
            tod_system.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("tod_edizm_COL")
            tod_edizm.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("tod_bug_COL")
            tod_bug.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("tod_valtype_COL")
...
''' </remarks>
        Public Overrides Sub XMLSaveCollections(ByVal node As System.Xml.XmlElement, ByVal Xdom As System.Xml.XmlDocument)
            tod_model.XMLSave(node, Xdom)
            tod_st.XMLSave(node, Xdom)
            tod_system.XMLSave(node, Xdom)
            tod_edizm.XMLSave(node, Xdom)
...
Public Overrides Sub BatchUpdate()
    tod_model.BatchUpdate
    tod_st.BatchUpdate
    tod_system.BatchUpdate
    tod_edizm.BatchUpdate
    tod_bug.BatchUpdate
