...
''' <summary>
''' Внутренняя переменная для раздела Расписание ТО
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_to_scheditems As to_scheditems_col


''' <summary>
'''Свойство для доступа к строкам раздела Расписание ТО
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property to_scheditems() As to_scheditems_col
            Get
                If m_to_scheditems Is Nothing Then
                    m_to_scheditems = New to_scheditems_col
                    m_to_scheditems.Application = Me
                    m_to_scheditems.Parent = Me
                    m_to_scheditems.Refresh()
                End If
                to_scheditems = m_to_scheditems
            End Get
        End Property
...
'''
''' </remarks>
  Private m_to_schedinfo As to_schedinfo_col
...
'''
''' </remarks>
        Public ReadOnly Property to_schedinfo() As to_schedinfo_col
            Get
                If m_to_schedinfo Is Nothing Then
                    m_to_schedinfo = New to_schedinfo_col
                    m_to_schedinfo.Application = Me
                    m_to_schedinfo.Parent = Me
                    m_to_schedinfo.Refresh()
                End If
                to_schedinfo = m_to_schedinfo
            End Get
        End Property
...
''' </remarks>
        Public Overrides Function GetDocCollection_Base(ByVal Index As Long) As DocCollection_Base
         ...
         Case 1
            return to_schedinfo
         Case 2
            return to_scheditems
            End Select
            return nothing
        End Function

        Public Overrides Sub Dispose()
            to_schedinfo.Dispose()
            to_scheditems.Dispose()
        End Sub
...
''' </remarks>
        ...
        FindInCollections = Nothing
            ...
            mFindInCollections = to_schedinfo.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = to_scheditems.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
        End Function
...
''' </remarks>
        Protected Overrides Sub XMLLoadCollections(ByVal node As System.Xml.XmlNode, Optional ByVal LoadMode As Integer = 0)
           ...
           try
            e_list = node.SelectNodes("to_schedinfo_COL")
            to_schedinfo.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("to_scheditems_COL")
            to_scheditems.XMLLoad(e_list, LoadMode)
...
''' </remarks>
        Public Overrides Sub XMLSaveCollections(ByVal node As System.Xml.XmlElement, ByVal Xdom As System.Xml.XmlDocument)
            to_schedinfo.XMLSave(node, Xdom)
            to_scheditems.XMLSave(node, Xdom)
        End Sub
...
Public Overrides Sub BatchUpdate()
    to_schedinfo.BatchUpdate
    to_scheditems.BatchUpdate
End Sub
    End Class
End Namespace
