...
''' <summary>
''' Внутренняя переменная для раздела Оператор
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_to_oper As to_oper_col


''' <summary>
'''Свойство для доступа к строкам раздела Оператор
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property to_oper() As to_oper_col
            Get
                If m_to_oper Is Nothing Then
                    m_to_oper = New to_oper_col
                    m_to_oper.Application = Me
                    m_to_oper.Parent = Me
                    m_to_oper.Refresh()
                End If
                to_oper = m_to_oper
...
''' </remarks>
        ...
        FindInCollections = Nothing
            ...
            mFindInCollections = to_oper.FindObject(Table, InstID)
...
''' </remarks>
        Protected Overrides Sub XMLLoadCollections(ByVal node As System.Xml.XmlNode, Optional ByVal LoadMode As Integer = 0)
           ...
           try
            e_list = node.SelectNodes("to_oper_COL")
            to_oper.XMLLoad(e_list, LoadMode)
