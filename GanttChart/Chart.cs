...
namespace Braincase.GanttChart
{
    ...
    public partial class Chart : UserControl
    {
        ...
        public Chart()
        {
            ...
            AllowDragDropModifications = false;
            ShowRelations = true;
            ShowSlack = false;
            ...
            Pen SBORDER = new Pen(Color.Red, 3); 
            // Formatting
            TaskFormat = new GanttChart.TaskFormat() {
                Color = Brushes.Black,
                Border = Pens.Maroon,
                SelectedBorder= SBORDER, 
        ...
        [DefaultValue(false)]
        public bool AllowDragDropModifications { get; set; }
        ...
        /// Occurs when a dragged Task is being dropped by releasing any previously pressed mouse button.
        /// </summary>
        public event EventHandler<TaskChangedEventArgs> TaskChanged = null;
        ...
        protected virtual void OnTaskMouseDrop(TaskDragDropEventArgs e)
        {
            ...
            // drop process disabled by client
            if (e.Cancel)
            {
                _mOverlay.Clear();
                this.Invalidate();
                    ...
                    if (Control.ModifierKeys.HasFlag(Keys.Shift))
                    {
                        if (AllowDragDropModifications)
                        {
                            ...
                            if (this.TryGetRow(source, out from))
                            {
                                ...
                                // Fire event
                                if (TaskChanged != null)
                                    TaskChanged(this, new GanttChart.TaskChangedEventArgs(e.Source));
                            }
                        }
                    }
                    else
        ...
        private void _GenerateModels()
        {
            ...
            foreach (var task in _mProject.Tasks)
            {
                if (!_mProject.AncestorsOf(task).Any(x => x.IsCollapsed))
                {
                    ...
                    taskRect = new RectangleF(task.Start * this.BarWidth, y_coord, task.Duration * this.BarWidth, this.BarHeight);
        ...
        private void __DrawRegularTaskAndGroup(Graphics graphics, TaskPaintEventArgs e, Task task, RectangleF taskRect)
        {
            ...
            if (_mSelectedTasks.Contains(task))
                // Draw border
                graphics.DrawRectangle(e.Format.SelectedBorder, taskRect);
            else
                // Draw border
                graphics.DrawRectangle(e.Format.Border, taskRect);
        ...
        private void __DrawTaskParts(Graphics graphics, TaskPaintEventArgs e, Task task, Pen pen)
        {
            ...
            if(_mSelectedTasks.Contains(task) )
                // Draw border
                graphics.DrawRectangles(e.Format.SelectedBorder, taskRects);
            else
                // Draw border
    ...
    public struct TaskFormat
    {
        ...
        public Pen SelectedBorder { get; set; }
        /// <summary>
    ...
    public class TaskChangedEventArgs : System.EventArgs
    {
        /// <summary>
        /// Subject Task of the event
        /// </summary>
        ...
        /// Initialize a new instance of TaskMouseEventArgs with the MouseEventArgs parameters and the Task involved.
        /// </summary>
        public TaskChangedEventArgs(Task task)
        {
            this.Task = task;
        }
    }
    ...
    {
        /// <summary>/// Get the chart that for this event
        /// </summary>
